...
The script takes a CSV file and builds a `.sql` file containing all the `INSERT` statements you need to later upload into phpMyAdmin.

From a terminal, go into the `script` folder and run:
```bash
python csv_loader.py path/to/your_file.csv > your_output.sql
```